...
function addRow(values = {}) {
  ...
  select.innerHTML = exercises
    .map(
      (exercise) =>
        `<option value="${exercise.id}">${escapeHtml(exercise.name)}${exercise.muscle_group ? ` · ${escapeHtml(exercise.muscle_group)}` : ''}</option>`,
    )
    .join('');
  for (const input of row.querySelectorAll('[data-name]'))
    if (values[input.dataset.name] !== undefined && values[input.dataset.name] !== null)
...
if (routineId) {
  ...
  document.querySelector('#page-description').textContent =
    'Revisa el plan actual y guarda los cambios que necesites.';
...
form.onsubmit = async (event) => {
  ...
  const exerciseRows = [...document.querySelectorAll('.exercise-row')].map((row) =>
    Object.fromEntries(
      [...row.querySelectorAll('[data-name]')].map((input) => [input.dataset.name, input.value]),
    ),
  );
