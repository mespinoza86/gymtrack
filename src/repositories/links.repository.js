...
export async function createInvitation(trainerId, code, expiresAt) {
  return (
    await pool.query(
      `INSERT INTO invitations (trainer_id, code, expires_at) VALUES ($1, $2, $3)
...
export async function listInvitations(trainerId) {
  return (
    await pool.query(
      `SELECT id, code, status, expires_at, accepted_at, created_at
    FROM invitations WHERE trainer_id = $1 ORDER BY created_at DESC`,
      [trainerId],
    )
  ).rows;
}
...
export async function acceptInvitation(code, athleteId) {
  return withTransaction(async (client) => {
    ...
    await client.query(
      `INSERT INTO trainer_athlete_links (trainer_id, athlete_id, status, ended_at)
      VALUES ($1, $2, 'active', NULL)
      ON CONFLICT (trainer_id, athlete_id) DO UPDATE SET status = 'active', ended_at = NULL`,
      [invitation.trainer_id, athleteId],
    );
    await client.query(
      `UPDATE invitations SET status = 'accepted', accepted_by = $2, accepted_at = NOW() WHERE id = $1`,
      [invitation.id, athleteId],
    );
    await client.query(
...
export async function listAthletes(trainerId) {
  return (
    await pool.query(
      `SELECT u.id, u.first_name, u.last_name, u.email, l.started_at,
    ...
    WHERE l.trainer_id = $1 AND l.status = 'active' ORDER BY u.first_name, u.last_name`,
      [trainerId],
    )
  ).rows;
}
...
export async function listTrainers(athleteId) {
  return (
    ...
    WHERE l.athlete_id = $1 AND l.status = 'active' ORDER BY u.first_name`,
      [athleteId],
    )
  ).rows;
}
...
export async function activeLink(trainerId, athleteId) {
  return Boolean(
    (
      await pool.query(
        `SELECT 1 FROM trainer_athlete_links WHERE trainer_id=$1 AND athlete_id=$2 AND status='active'`,
        [trainerId, athleteId],
      )
    ).rowCount,
  );
