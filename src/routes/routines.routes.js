...
const routineInput=z.object({athleteId:z.union([z.uuid(),z.null()]).optional(),name:z.string().trim().min(2).max(140),description:z.string().max(3000).optional(),status:z.enum(['draft','active','archived']).default('draft'),startDate:z.string().optional(),endDate:z.string().optional(),days:z.array(z.object({name:z.string().trim().min(1).max(100),notes:z.string().max(1000).optional(),exercises:z.array(exerciseItem).min(1)})).min(1)});
const customExerciseInput=z.object({name:z.string().trim().min(2).max(140),muscleGroup:z.string().trim().max(80).optional(),instructions:z.string().trim().max(3000).optional(),mediaUrl:z.union([z.url(),z.literal('')]).optional()});
...
router.post('/exercises',requireRole('trainer'),validate(customExerciseInput),asyncHandler(controller.createExercise));
router.put('/exercises/:id',requireRole('trainer'),validate(customExerciseInput),asyncHandler(controller.updateExercise));
router.put('/exercises/:id/status',requireRole('trainer'),validate(z.object({isActive:z.boolean()})),asyncHandler(controller.setExerciseStatus));
...
router.post('/',requireRole('trainer'),validate(routineInput),asyncHandler(controller.createRoutine));
router.get('/history',requireRole('athlete'),asyncHandler(controller.history));
router.get('/:id',asyncHandler(controller.routine));
router.put('/:id',requireRole('trainer'),validate(routineInput),asyncHandler(controller.updateRoutine));
